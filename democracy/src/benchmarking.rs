...
fn get_alice<T: Config>() -> T::AccountId {
	...
	T::MultiCurrency::deposit(T::GetNativeCurrencyId::get(), &alice, (100000 * DOLLARS).saturated_into::<BalanceOf<T>>());
	alice
}

fn get_dao_account<T: Config>(second_id: T::SecondId) -> T::AccountId {
	let who = second_id.into_account();
	T::MultiCurrency::deposit(T::GetNativeCurrencyId::get(), &who, (100000 * DOLLARS).saturated_into::<BalanceOf<T>>());
	who
}
...
fn creat_dao<T: Config>() -> (T::DaoId, T::SecondId) {
	...
	assert!(dao::Pallet::<T>::create_dao(SystemOrigin::Signed(alice).into(), dao_id, second_id.clone()).is_ok());
	(dao_id, second_id)
}
...
fn create_proposal<T: Config>() -> (T::DaoId, T::SecondId, ProposalIndex) {
	...
	assert!(Democracy::<T>::propose(SystemOrigin::Signed(get_alice::<T>()).into(), dao_id, Box::new(proposal), amount).is_ok());
...
fn vote1<T: Config>() -> (T::DaoId, T::AccountId, ProposalIndex) {
	...
	assert!(Democracy::<T>::vote(SystemOrigin::Signed(dao.clone()).into(), dao_id, index, T::Vote::default(), T::Conviction::default(), Attitude::AYES).is_ok());
	(dao_id, dao, index)
}
...
fn enact<T: Config>() -> T::AccountId {
	...
	assert!(Democracy::<T>::enact_proposal(SystemOrigin::Signed(dao_account.clone()).into(), dao_id, index).is_ok());
	dao_account
}
