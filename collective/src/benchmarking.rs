use frame_benchmarking::{account, benchmarks, benchmarks_instance_pallet, benchmarks_instance, impl_benchmark_test_suite, whitelisted_caller};
...
use crate::Call as CollectiveCall;
use super::*;
...
fn create_dao<T: Config<I>, I: 'static>() -> (T::DaoId, T::SecondId) {
	...
	assert!(dao::Pallet::<T>::create_dao(SystemOrigin::Signed(get_alice::<T, I>()).into(), dao_id, second_id.clone()).is_ok());
	CollectiveMembers::<T, I>::insert(dao_id, vec![get_alice::<T, I>(), get_dao_account::<T, I>(second_id.clone())]);
	(dao_id, second_id)
}

fn get_proposal<T: Config<I>, I: 'static>(dao_id: T::DaoId) -> (T::Proposal, T::Hash) {
	let proposal: T::Proposal = DaoCall::<T>::dao_remark {
			dao_id: dao_id,
			remark: vec![1; 20],
		}.into();
...
fn create_proposal<T: Config<I>, I: 'static>() -> (T::DaoId, T::SecondId, T::Hash, ProposalIndex) {
	...
	assert!(Collective::<T, I>::propose(SystemOrigin::Signed(get_alice::<T, I>()).into(), dao_id, 2 as ProposalIndex, Box::new(proposal)).is_ok());
...
fn user_vote<T: Config<I>, I: 'static>() -> (T::DaoId, T::SecondId, T::Hash, ProposalIndex) {
	...
	assert!(Collective::<T, I>::vote(SystemOrigin::Signed(dao_account).into(), dao_id, proposal_hash, index, true).is_ok());
